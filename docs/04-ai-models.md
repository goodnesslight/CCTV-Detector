...
### 4.4.6. Embedding'и в журналі подій (пошук по обличчю)

Той самий 128-вимірний вектор, що генерується для cosine-матчу з whitelist'ом,
**зберігається** разом з подією `unknown_face` у БД (колонка
`events.face_embedding BLOB`). Це дозволяє реалізувати функцію **пошуку по
обличчю** в журналі подій:

```
користувач завантажує фото
        │
        ▼
FaceRecognizer.embedding_for_image(path) → query 128-D
        │
        ▼
SELECT * FROM events WHERE face_embedding IS NOT NULL
        │
        ▼
np.stack(embs) → (N, 128) ; embs @ query → (N,) cosine
        │
        ▼
filter sim ≥ 0.4, sort desc → результати
```

Розмір на одну подію — 128 × 4 байти = **512 байт**. На 10 000 подій — 5 MB,
що тривіально. Швидкість пошуку — векторизований matrix-vector multiply,
~50 мс на 10K подій.

Поріг 0.4 узгоджений з порогом класифікації `known_face` — той самий, що
використовується у real-time recognition.
